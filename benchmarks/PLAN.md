...
**Peak RSS + CPU:** the **child crawler process itself** reads resource usage at the `go` checkpoint and again immediately before printing its result JSON — never the orchestrator, and never `RUSAGE_CHILDREN` (which aggregates all reaped children and cannot be attributed to one trial). Peak RSS remains the absolute process high-water mark and is normalized to bytes (macOS `getrusage` reports bytes; Linux reports KiB; Node reports KiB). Published `ru_utime`/`ru_stime` CPU is the final-minus-`go` delta, so module/runtime startup before the timed region is not charged. Rust and Go use `getrusage(RUSAGE_SELF)`; Node uses `process.resourceUsage()`. A tracking allocator is rejected (measures heap, not RSS; perturbs allocation behavior). Parent-side sampling is not used (it cannot attribute language-runtime memory more accurately than child self-report).

**Cross-language runtime model:** `--runtime-workers` sets both Rust's Tokio worker count and Go's `GOMAXPROCS` (4 by default). Crawlee's Cheerio handlers and parsing run on Node's single JavaScript event-loop thread; native runtime helpers remain implementation details. The per-sample `runtime_workers` field records 4 for Rust/Go and 1 for Crawlee. HTTP fetch concurrency remains the identical primary control (`C = 32`) for every crawler.
...
| A-3 | Same integer C ≠ same unit of work (spider's subscriber runs outside its permit pool) | Cannot be perfectly equalized without distorting one engine's architecture. Mitigations: identical fixed 4-worker Tokio runtime caps the two Rust engines equally; Go uses the same `GOMAXPROCS=4`; Crawlee's JavaScript work remains single-threaded and is disclosed separately; single spider drain task (parse parallelism 1, no per-page spawning); timed CPU (user+sys) is published so total work is visible independently of where it runs. |
